//! sys_menu facade — 範式 B（樹狀 entity、FR-026 tree-cascade）
//!
...
//!
//! PK 型別為 `i32`、pid 型別為 `String`（cascade check 需 `id.to_string()` 轉換）。
...
use crate::admin::entities::sys_menu as _entity;
...
pub async fn soft_delete_by_id<C>(db: &C, id: i32, actor: &Actor) -> Result<(), AppError>
where
    C: ConnectionTrait + TransactionTrait,
{
    ...
    // F2.1: fetch before snapshot（active row state；cascade check 已通過）
...
pub async fn restore_by_id<C>(db: &C, id: i32, actor: &Actor) -> Result<(), AppError>
where
    C: ConnectionTrait + TransactionTrait,
{
    let txn = db.begin().await.map_err(|e| AppError {
